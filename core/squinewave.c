...
/*
    ...
    csoundport: the Csound 7 code with the API removed.
*/
...
static inline int32_t find_sync(const double* sync_sig, const uint32_t first,
                                const uint32_t last)
{
    uint32_t i;
    if (sync_sig == 0)
...
static void hardsync_init(sqw_osc *p, const double freq,
...
int32_t sqw_init(sqw_osc *p, double sr, double iminsweep, double iphase)
{
    int32_t warned = 0;
    p->iphase = iphase;
    ...
    p->Min_Sweep = iminsweep;

    // Allow range 4-sr/100
    if (p->Min_Sweep < 4.0 || p->Min_Sweep > sr * 0.01) {
      const double minsweep_default = Clamp(sr / 3000.0, 8.0, sr * 0.01);
      if (p->Min_Sweep != 0.0)
        warned = 1;
...
void sqw_perform(sqw_osc *p, const double *freq_sig, const double *clip_sig,
                 const double *skew_sig, const double *sync_sig, double *aout,
                 double *async_out, uint32_t nsmps)
{
    uint32_t n;
    const uint32_t ksmps_offset = 0;
    const uint32_t ksmps_end = nsmps;
    ...
    int32_t sync = find_sync(sync_sig, ksmps_offset, ksmps_end);
    ...
    if (async_out)
      memset(async_out, 0, nsmps * sizeof(double));
      ...
      if (sweep_phase >= 2.0 && phase >= 2.0)
        {
            if (hardsync_phase) {
              ...
              sync = find_sync(sync_sig, n + 1, ksmps_end);
            }
            else {
              phase -= 2.0;
              if (phase > phase_inc) {
